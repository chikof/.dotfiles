...
local function config_file_matcher(buf)
    return 
        vim.api.nvim_buf_get_name(buf.id):match("Cargo.toml")
...
return {
    ...
    keys = function()
        return {
            { "<A-h>", "<cmd>BufferLineCyclePrev<cr>", desc = "Previous buffer", mode = { "n", "i" } },
            { "<A-l>", "<cmd>BufferLineCycleNext<cr>", desc = "Next buffer", mode = { "n", "i" } },
            {
                "<A-S-h>",
